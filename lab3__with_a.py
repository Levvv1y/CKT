...
def initValues(number, theta):
    ...
    thetaTrue = np.array([-0.8, 1])     # истинные значения параметров
    
    if number == 4:
        return thetaTrue, N, q
    
    F =  np.array([[theta[0]/10,1],[0.1,0]])     # матрица процесса системы (матрица перехода)
    psi =  np.array([[1], [theta[1]]])         # матрица управления
    G =  np.array([[1], [1]])
